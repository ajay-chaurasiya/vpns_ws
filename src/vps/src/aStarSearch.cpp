...
// been reached or not
bool aStarSearch::isDestination(int row, int col, Pair dest, int &xo, int &yo, int &xn, int &yn, int &dx, int &dy, int &ao, int &xc, int &yc, ros::Publisher &pub)
{
    xo = dx = dest.first;
    yo = dy = dest.second;
    navigation::positionDetails(xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
                        ...
                        Pair &dest, cell cellDetails[][COL], set<pPair> &openList, bool closedList[][COL], int grid[][COL], bool &foundDest, cv::Mat &map, cv::Mat &R, float PtoW[4][4], int &xo, int &yo, int &xn, int &yn, int &dx, int &dy, int &ao, int &xc, int &yc, ros::Publisher &pub)
{
    ...
    directionProcess(i, j, k, l, gNew, hNew, fNew, dest, cellDetails, openList, closedList, grid, foundDest, map, R, PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
}
                                   ...
                                   Pair &dest, cell cellDetails[][COL], set<pPair> &openList, bool closedList[][COL], int grid[][COL], bool &foundDest, cv::Mat &map, cv::Mat &R, float PtoW[4][4], int &xo, int &yo, int &xn, int &yn, int &dx, int &dy, int &ao, int &xc, int &yc, ros::Publisher &pub)
{
    ...
    if (isValid(k, l))
    {
        ...
        // current successor
        if (isDestination(k, l, dest, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub))
        {
            // Set the Parent of the destination cell
            cellDetails[k][l].parent_i = i;
            cellDetails[k][l].parent_j = j;
            printf ("The destination cell is found\n");
            tracePath (cellDetails, dest, map, R, PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
            foundDest = true;
            return;
        }
...
// to destination
void aStarSearch::tracePath(cell cellDetails[][COL], Pair &dest, cv::Mat &map, cv::Mat &R, float PtoW[4][4], int &xo, int &yo, int &xn, int &yn, int &dx, int &dy, int &ao, int &xc, int &yc, ros::Publisher &pub)
{
...
// to A* Search Algorithm
    aStarSearch::aStarSearch(int grid[][COL], Pair &src, Pair &dest, cv::Mat &map, cv::Mat &R, float PtoW[4][4], int &ao, int &xc, int &yc, ros::Publisher &pub)
{
    ...
    if (isDestination(src.first, src.second, dest, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub))
    {
    ...
    while (!openList.empty())
    {
        ...
        if (!foundDest)
            north(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
        if (!foundDest)
            south(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
        if (!foundDest)
            east(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
        if (!foundDest)
            west(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
/*        if (!foundDest)
            northEast(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
        if (!foundDest)
            northWest(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
        if (!foundDest)
            southEast(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);
        if (!foundDest)
            southWest(i,j,gNew,hNew,fNew,dest,cellDetails,openList,closedList,grid,foundDest,map,R,PtoW, xo, yo, xn, yn, dx, dy, ao, xc, yc, pub);	*/
    }
